...
def check_password(): 
    ...
    def password_entered(): 
        ...
        print(f"Secret Password: {st.secrets['MMPW']}")  # Added print statement 
        print(f"User Password: {password}")             # Added print statement 
...
PREBUILT_COMMANDS = {
    "/aphgfrq": {
        "title": "/aphgfrq",
        "description": "Paste an FRQ prompt or prompts.",
        "prompt": "Answer the following prompt or prompts. Label with Question Number (if present) and the question title. Use human geography vocabulary exceptionally and provide a 5/5 detailed, student-like response, 9th-10h grade APHG level, still minorly-collegiate response type. ALWAYS ALWAYS ALWAYS answer the question based on the task verb, there may be additional sentences before but the last sentence will contain a task verb. For questions with IDENTIFY, just provide the information. Name it in only one sentence simply. For questions with DEFINE, use the WEE define method (3 sents, to a paragraph) -> Write the defintiion, give an Example (geographic example or conceptual), and Explain how the example meets the definition. For questions with DESCRIBE, provide relevant characteristics, make connectons. Use several detailed sentences/ Always explain the details. For questions with EXPLAIN, ACE the question. Answer the question, Cite a geographic example or location, and Explain the exsmple. For questions with Explain to the extnet/degree of, always name a degree (hgih/low/moderate) in the first sentene when answering the questions, thewn explain in 2-3 more sentences why/how it is to a greater/lesser degree, for questions with COMPARE, prvoide at least 3 difference, each difference should have one part stating and one part explaining. USe transition words to explain both sides. ALWAYS ALWAYS ALWAYS (unless identify) define key human geography terms before using them, use correct accurate definitions that make sense. ALWAYS go for 3-4 sentences. ONly if ABSOLUTELY DEFINITELY Nnecessary, use a fifth or sixth sentence, but try not to.",
        "message_text": " ----- **Active Command:** Human Geography Free-Response ----- ", 
    },
    "/weeklyhgflashcards": {
